...
Единственным источником версии является поле `project.version` в `pyproject.toml`. CD запускается только для тегов формата `v*.*.*`, повторяет все проверки качества и проверяет совпадение тега с версией проекта.

Для версии `0.1.0` публикация запускается командами:
...
После успешных проверок CD собирает Docker-образ и публикует его в GitHub Container Registry:
